...
from Codes.Utilities import saveimages, poissonScaling, kdemzs
...
if 'colors' not in plt.colormaps():
    ...
    colors = [(0.05, 0.05, 0.05, 0.0), # background
              (0.65, 0.65, 0.65, 1.0), # wm
              (0.15, 0.15, 0.15, 1.0), # gm
              (0.65, 0.15, 0.15, 1.0), # injury
              # (0.15, 0.15, 0.75, 1.0),
              # (0.95, 0.75, 0.05, 1.0),
...
for idx, l in enumerate(labels):
    if l == 2:
        gmSpectra.append(ionImageSpectra[idx])
    if l == 3:
        injuredSpectra.append(ionImageSpectra[idx])
    if l == 1:
        wmSpectra.append(ionImageSpectra[idx])
...
if __name__ != '__main__':
    gmSpectra = np.array(gmSpectra)
    injuredSpectra = np.array(injuredSpectra)
    wmSpectra = np.array(wmSpectra)
    ...
    plt.plot(peakmzs, gmMean, 'tab:gray', label='gm', alpha=0.5)
    plt.plot(peakmzs, wmMean, 'wheat', label='wm', alpha=0.5)
    plt.plot(peakmzs, injuredMean, 'tab:red', label='injured', alpha=0.5)
    plt.legend(fontsize=20)
    plt.show()

    plt.fill_between(peakmzs, 0, gmMedian, label='gm', color='tab:gray', alpha=0.4, hatch=None, edgecolor='k', zorder=1)
    plt.fill_between(peakmzs, 0, wmMedian, label='wm', color='wheat', alpha=0.4, hatch=None, edgecolor='k', zorder=1)
    plt.fill_between(peakmzs, 0, injuredMedian, label='injured', color='tab:red', alpha=0.4, hatch=None, edgecolor='k', zorder=-1)

    plt.legend(fontsize=12)
    plt.xlabel('Peak MZs', fontsize=14)
    plt.ylabel('Median', fontsize=14)
    plt.title('Change in intensity', fontsize=16)
    plt.show()
...
reg1specIdx = []
for df in [gm1df, wm1df]:
    reg1specIdx.extend(df['Spot index'].values)
print(len(reg1specIdx))
