...
plt.show()
#
# plt.vlines(mzs, 0, loadingsMatrixDF['PC01'].values, label='loading 1', colors='b', linestyles='solid', alpha=0.5)
# plt.vlines(mzs, 0, loadingsMatrixDF['PC02'].values, label='loading 2', colors='g', linestyles='solid', alpha=0.5)
# plt.vlines(mzs, 0, loadingsMatrixDF['PC03'].values, label='loading 3', colors='r', linestyles='solid', alpha=0.5)
# plt.vlines(mzs, 0, (regSpecNorm[40]-min(regSpecNorm[40]))/(max(regSpecNorm[40])-min(regSpecNorm[40])),
#            label='sample spec', colors='k', linestyles='solid', alpha=0.5)
# plt.legend()
# plt.show()
...
# saveimages(pcaScoreImages[min(xCor):max(xCor)+1, min(yCor):max(yCor)+1, :],
#            pca.explained_variance_ratio_[0:nPCs],
#            r'/media/banikr/DATA/MALDI/230713-Chen-intactproteins/pcaimages/')
...
for i in range(nPCs):
    ...
    axes[i, 1].vlines(peakmzs, 0, loadingsMatrixDF.values[:, i], label='loading {}'.format(i+1), colors='k', linestyles='solid', alpha=0.5)
...
componentMatrixVar = favarimax.components_.T
print(componentMatrixVar.shape)

varScoreImages = np.zeros([max(xCor)+1, max(yCor)+1, nPCs])
for idx, spot in enumerate(specIdx):
    varScoreImages[xCor[spot], yCor[spot], :] = varScores[idx, 0:nPCs]

fig, axes = plt.subplots(nPCs, 2, figsize=(15, 25))
# Loop through the subplots and plot images and graphs
for i in range(nPCs):
    # Plot images in the first column
    rotated_image = np.rot90(varScoreImages[min(xCor):max(xCor)+1, min(yCor):max(yCor)+1, i])
    axes[i, 0].imshow(rotated_image,
                      cmap='coolwarm')
    # axes[i, 0].set_title('{:.3f}'.format(pca.explained_variance_ratio_[i]))
    axes[i, 0].axis('off')  # Turn off axis ticks and labels
    ...
    # Plot graphs in the second column
    # axes[i, 1].plot(data_graphs[i], label=f'Graph {i + 1}')
    axes[i, 1].vlines(peakmzs, 0, componentMatrixVar[:, i], label='loading {}'.format(i+1), colors='k', linestyles='solid', alpha=0.5)
    # axes[i, 1].set_title(f'Graph {i + 1}')
